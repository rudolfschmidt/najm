...
import com.rudolfschmidt.najm.repositories.SimpleModelRepository;
...
public class PersistenceWithSimpleModelTest {

	private final static SimpleModelRepository repository;

	static {
		MongoDatabase mongoDatabase = new MongoClient().getDatabase(Constants.DATABASE_NAME);
		repository = MongoRepository.newInstance(SimpleModelRepository.class, mongoDatabase);
	}
	...
	public void findOne() {
		SimpleModel expected = SimpleModel.newInstance();
		repository.save(expected);
		SimpleModel actual = repository.findOne(expected.getId());
